...
uint64_t AlgebraicFactorHelper::RemoveTermsWithAlgebraicFactors(seq_t *seq)
{
   ...
   CheckForSpecialForm(seq);

   removedCount = RemoveSimpleTerm(seq);
   
   if (startingCount - removedCount > 0)
      removedCount += RemoveTermsWithKPowers(seq);
   
   if (startingCount - removedCount > 0)
      removedCount += RemoveTermsWithKAndBPowers(seq);
   
   if (startingCount - removedCount > 0)
      removedCount += RemoveComplexRoot(seq);
         
   if (startingCount - removedCount > 0)
      removedCount += CheckPower4(seq);

   if (startingCount - removedCount > 0)
      removedCount += CheckBase2(seq);
...
uint64_t  AlgebraicFactorHelper::RemoveSimpleTerm(seq_t *seq)
{
   ...
   if (seq->k == 1)
   {
      ...
      ip_App->WriteToConsole(COT_OTHER, "(st) Sequence has algebraic factorization: %" PRIu64"*%u^n%+d -> b = %u^%u", seq->k, ii_Base, seq->c,
               ii_BRoot, ii_BPower);
   }
   else 
   {
      ...
      ip_App->WriteToConsole(COT_OTHER, "(st) Sequence has algebraic factorization: %" PRIu64"*%u^n%+d -> k = %u^%u and b = %u^%u", seq->k, ii_Base, seq->c,
   ...
   for (n=ii_MinN; n<=ii_MaxN; n++)
   {
      ...
      removedCount += CheckAndLogAlgebraicFactor(seq, n, "%u", ii_BRoot + seq->c);
   }
   
   ip_App->WriteToConsole(COT_OTHER, "(st) Sequence %" PRIu64"*%u^n%+d has %d terms removed has they have the factor %d", 
...
uint64_t  AlgebraicFactorHelper::RemoveTermsWithKPowers(seq_t *seq)
{
   ...
   // seq->k must be x^f for some x and f must be greater than 1
   if (ii_KPower == 1)
      return 0;
   ...
   for (idx=1; idx<ii_KPower; idx++)
   {
      ...
      ip_App->WriteToConsole(COT_OTHER, "(kp) Sequence has algebraic factorization: %" PRIu64"*%u^n%+d -> (%u^%u)*%u^n%+d", seq->k, ii_Base, seq->c,
      ...
      for (n=ii_MinN; n<=ii_MaxN; n++)
      {
         ...
         loopRemovedCount += CheckAndLogAlgebraicFactor(seq, n, "(%u*%u^%u%+d)", curroot, ii_Base, n/curpower, seq->c);
      }

      ip_App->WriteToConsole(COT_OTHER, "(kp) Sequence %" PRIu64"*%u^n%+d has %d terms removed due to algebraic factors of the form %u*%u^(n/%u)%+d", 
...
// If c=-1, z divides f and n*g, any n*g, then x^(f/z)*y^((n*g)/z)-1 will be a factor.
// If c=+1, z divides f and n*g, odd n*g, then x^(f/z)*y^((n*g)/z)+1 will be a factor.
uint64_t  AlgebraicFactorHelper::RemoveTermsWithKAndBPowers(seq_t *seq)
{
   uint32_t  removedCount = 0, loopRemovedCount = 0;
   uint32_t  n, idx;
   uint32_t  curroot, curpower;
   ...
   for (idx=2; idx<=ii_KPower; idx++)
   {
      ...
      if (idx == ii_KPower)
        ip_App->WriteToConsole(COT_OTHER, "(kbp) Sequence %" PRIu64"*%u^n%+d has %d terms removed due to algebraic factors of the form %u*%u^((%u*n)/%u)%+" PRId64"", 
                  seq->k, ii_Base, seq->c, loopRemovedCount, curroot, ii_BRoot, ii_BPower, curpower, seq->c);
      else
        ip_App->WriteToConsole(COT_OTHER, "(kbp) Sequence %" PRIu64"*%u^n%+d has %d terms removed due to algebraic factors of the form (%u^%u)*%u^((%u*n)/%u)%+" PRId64"", 
...
// If seq->k*b^ii = root^m for root > 1 and m > 1, then we can remove algebraic factors
...
uint64_t  AlgebraicFactorHelper::RemoveComplexRoot(seq_t *seq)
{
   ...
   // base and seq->k must share a divisor
   if (gcd64(ii_Base, seq->k) == 1)
      return 0;
   ...
   // Step 2:  Factorize k
   kf_count = GetFactorList(seq->k, kf_factor, kf_power);
   ...
   for (ii=1; ii<10; ii++)
   {
      // We could multiply out seq->k*base^ii and factor that value, but
      // seq->k*base^ii will likely exceed what can be stored in a 64-bit integer.
      ...
      // We now have seq->k*base^ii = f1^p1 * f2^p2 ... fn^pn
      ...
      // If m then seq->k*base^ii is not a perfect power
      if (m == 1)
         continue;
      ...
      {         
         ...
         {
            if (ii == 1)
              ip_App->WriteToConsole(COT_OTHER, "(cr) Sequence has algebraic factorization: %" PRIu64"*%u^n%+" PRId64" -> %" PRIu64"*%u = (%s)^%u", seq->k, ii_Base, seq->c,
                        seq->k, ii_Base, root, z);
            else
              ip_App->WriteToConsole(COT_OTHER, "(cr) Sequence has algebraic factorization: %" PRIu64"*%u^n%+" PRId64" -> %" PRIu64"*%u^%u = (%s)^%u", seq->k, ii_Base, seq->c,
                        seq->k, ii_Base, ii, root, z);
         }
         else
         {
            if (ii == 1)
         ...
         if (!strcmp(root, "2") && seq->c == -1 && n-ii==1) n++;
...
uint64_t  AlgebraicFactorHelper::CheckBase2(seq_t *seq)
{
   uint64_t  k;
   uint32_t  removedCount = 0;
   ...
   for ( ; n<=ii_MaxN; n+=4)
      removedCount += CheckAndLogAlgebraicFactor(seq, n, "(%u^2*2^%u-%u*2^%u+1)",
...
uint64_t  AlgebraicFactorHelper::CheckPower4(seq_t *seq)
{
   uint32_t  removedCount = 0;
   uint32_t  n, ninc;
   uint32_t  b1, bexp;
   ...
   {
      // Now we have seq->k = 4*ii_KRoot^ii_KPower
      GetRoot(seq->k/4, &ii_KRoot, &ii_KPower);
   ...
   for (; n<=ii_MaxN; n+=ninc)
   {
      ...
      {
         if (ninc == 1)
            removedCount += CheckAndLogAlgebraicFactor(seq, n, "(2*(%u^%u)^%u+2*(%u^%u)^%u+1)",
                  ii_BRoot, ii_BPower/2, n, ii_BRoot, ii_BPower/4, n);
         else if (ninc == 2)
            removedCount += CheckAndLogAlgebraicFactor(seq, n, "(2*%u^%u+2*%u^%u+1)",
      ...
      {
         if (ninc == 1)
            removedCount += CheckAndLogAlgebraicFactor(seq, n, "(2*%u^%u*(%u^%u)^%u+2*%u^%u*(%u^%u)^%u+1)",
                  ii_KRoot, ii_KPower/2, ii_BRoot, ii_BPower/2, n, ii_KRoot, ii_KPower/4, ii_BRoot, ii_BPower/4, n);
         else if (ninc == 2)
            removedCount += CheckAndLogAlgebraicFactor(seq, n, "(2*%u^%u*%u^%u+2*%u^%u*%u^%u+1)",
   ...
   if (removedCount)
   {
      ...
      {
         if (ninc == 1)
           ip_App->WriteToConsole(COT_OTHER, "(p4) Removed %d algebraic factors for %" PRIu64"*%u^n%+d of the form (2*(%u^%u)^n+2*(%u^%u)^n+1)",
                  removedCount, seq->k, ii_Base, seq->c, ii_BRoot, ii_BPower/2, ii_BRoot, ii_BPower/4);
         else if (ninc == 2)
           ip_App->WriteToConsole(COT_OTHER, "(p4) Removed %d algebraic factors for %" PRIu64"*%u^n%+d of the form (2*(%u^%u)^n+2*(%u^%u)^n+1)",
                  removedCount, seq->k, ii_Base, n, seq->c, ii_BRoot, ii_BRoot, n/2);
         else
           ip_App->WriteToConsole(COT_OTHER, "(p4) Removed %d algebraic factors for %" PRIu64"*%u^n%+d of the form (2*%u^(n/2)+2*%u^(n/4)+1)",
                  removedCount, seq->k, ii_Base, seq->c, ii_BRoot, ii_BRoot);
      }
      else
      {
         if (ninc == 1)
           ip_App->WriteToConsole(COT_OTHER, "(p4) Removed %d algebraic factors for %" PRIu64"*%u^n%+d of the form (2*%u^%u*(%u^%u)^n+2*%u^%u*(%u^%u)^n+1)",
           ...
           ip_App->WriteToConsole(COT_OTHER, "(p4) Removed %d algebraic factors for %" PRIu64"*%u^n%+d of the form (2*%u^%u*%u^(n/2)+2*%u^%u*%u^(n/4)+1)",
...
{   
   ...
   if (!seq->nTerms[n-ii_MinN])
      return 0;
   
   seq->nTerms[n-ii_MinN] = false;

   if (!ip_AlgebraicFactorFile)
   {   
      char  fName[50];
      
      sprintf(fName, "alg_%" PRIu64"_%u_%+" PRId64".log", seq->k, ii_Base, seq->c);
...
// If c = -1 and k=2^f and b=2^g for any f and g, then this is a Mersenne number.
// If c = 1 and k=x^f and b=x^g then we have a Generalized Fermat Number.
bool     AlgebraicFactorHelper::IsGfnOrMersenneForm(seq_t *seq)
{   
   // Needs to be k*b^n-1 or k*b^n+1
   if (seq->c != -1 && seq->c != 1)
      return false;
